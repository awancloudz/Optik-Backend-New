...
class TransaksiController extends Controller
{
    ...
    public function view($idtrans){
        ...
        $data = Transaksi::where('id', $idtrans)->with('customer')->with('karyawan')->with('detailtransaksi.produk.merk')->with('pengiriman.profile')->orderBy('created_at', 'desc')->get();
        $jumlah = $data->count();
        if($jumlah > 0){
            $transaksi = collect($data);
            $transaksi->toJson();
    ...
    public function store(Request $request){
        ...
        settype($idkaryawan, "integer");
            ...
            if($transaksiakhir->count() > 0){
                $kodetransaksi = $trx . $idkaryawan . "-" . sprintf("%010s", $kodeakhir->id + 1);
            }
            else
            {
                $kodetransaksi = $trx . $idkaryawan . "-0000000001";
            }
    ...
    public function destroy($id){
        ...
        foreach($detailtransaksi as $detail){
            ...
            //Update stok Cabang
                ...
                $stokakhir = $stokawal + $detail->jumlah;
                $produkcabang = StockCabang::where('id_produk',$detail->id_produk)->where('id_profile',$detail->id_cabang)->get();
                foreach($produkcabang as $produk2){
                    $stokawal2 = $produk2->stok;
